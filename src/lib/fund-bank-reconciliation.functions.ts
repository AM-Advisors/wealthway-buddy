...
async function matchDeclaredPayments(supabase: any, userId: string, offeringId: string) {
  ...
  for (const line of (lines ?? []) as any[]) {
    ...
    const hit = open.find((inv) => {
      ...
      return Boolean(inv.client_paid_on) && daysApart(String(inv.client_paid_on), String(line.posted_on)) <= 6;
    });
    if (!hit) continue;
