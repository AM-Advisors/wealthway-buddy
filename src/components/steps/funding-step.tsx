...
export function FundingStep({ offeringId }: { offeringId?: string }) {
  useStepView("funding");
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["funding", offeringId ?? "active"],
  ...
  useEffect(() => {
    const refresh = () => {
      void queryClient.invalidateQueries({ queryKey: ["funding", offeringId ?? "active"] });
    };
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["funding", offeringId ?? "active"] });
  const onError = (e: Error) => toast.error(e.message);

  const chooseWireMutation = useMutation({
    mutationFn: () => wire({ data: scope }),
    onSuccess: () => {
  ...
  const acknowledgeMutation = useMutation({
    mutationFn: (m: "wire" | "ach") =>
      acknowledge({
        data: { method: m, statements: m === "wire" ? WIRE_STATEMENTS : ACH_STATEMENTS, ...scope },
      }),
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <h1 className="mt-2 text-3xl">Fund your subscription</h1>
